...
def deck_get():
    ...
    if 'q' in request.GET.keys():
        ...
        if gledena == "id":
            ...
            cur.execute("SELECT * FROM deck where substring(lower(ime) from (%s)) IS NOT NULL",[kaj.lower()])
        else:
            cur.execute("SELECT * FROM deck where substring(lower(avtor) from (%s)) IS NOT NULL",[kaj.lower()])
